...
.DESCRIPTION
  ランダムなパスワードを生成し、CSVファイルの他、いくつかの形式でデータを保存できます。
  PowerShell 3.0以降が推奨です。
  PowerShell 2.0でも動作しますが、明示的にシングルスレッド アパートメント (STA) モードを指定する必要があります。
  ※PowerShell 3.0以降ではSTAモードが既定のため指定は不要です。
.EXAMPLE
  PowerShell.exe -Sta -File PasswordGenerator.ps1
  コマンドプロンプトからSTAモードでプログラムを起動する。
#>
...
$Form1.Size = New-Object System.Drawing.Size(610,350)
...
$NumberBox1.Location = New-Object Drawing.Size(25,25)
$NumberBox1.Size = New-Object Drawing.Size(80,20)
...
$GenerateSettingsLoad = @'
	...
	{
		if ($CheckBoxesCount -eq $NumberBox1.Text)
		{
			$EachCharCount = 1
		}
		else
		{
			$EachCharCount = 1
			$RandomCount = 1..($NumberBox1.Text - $CheckBoxesCount)
		}
	}
	elseif ($ComboBox1.SelectedIndex.Equals(1))
	{
		...
		{
			if ((($NumberBox1.Text - $CheckBoxesCount) % ($CheckBoxesCount -1)) -eq 0)
			{
				$EachCharCount = ($NumberBox1.Text - $CheckBoxesCount) / ($CheckBoxesCount -1) + 1
				$RandomCount = 0
			}
			else
			{
				$EachCharCount = [Math]::Round(($NumberBox1.Text - $CheckBoxesCount) / ($CheckBoxesCount)) + 1
				$RandomCount = 1..(($NumberBox1.Text - $CheckBoxesCount) % ($CheckBoxesCount -1))
			}
		}
		else
		{
			if ((($NumberBox1.Text - $CheckBoxesCount) % ($CheckBoxesCount)) -eq 0)
			{
				$EachCharCount = [Math]::Round(($NumberBox1.Text - $CheckBoxesCount) / ($CheckBoxesCount)) + 1
				$RandomCount = 0
			}
			...
			{
				$EachCharCount = [Math]::Round(($NumberBox1.Text - $CheckBoxesCount) / ($CheckBoxesCount)) + 1
				$RandomCount = 1..(($NumberBox1.Text - $CheckBoxesCount) % ($CheckBoxesCount))
			}
		}
	}
	else
	{
		$RandomCount = 1..$NumberBox1.Text
	}
'@
$Button5.Add_Click({
	$CheckBoxesCount = checkboxescount
	Invoke-Expression ($GenerateSettingsLoad -Join "`r`n")
	$strChars = strcharscreate
	$transcommand = [Scriptblock]::Create((transcommandgenerate))
	$TextBox1.Text = $GeneratePassword.Invoke()
	$TextBox2.Text = $GetRead.Invoke($TextBox1.Text)
...
$Button15.Add_Click({
	...
	if ($RadioButton1.Checked)
	{
		...
		$ProgressBar1.Maximum = $NumberBox2.Text
		$ProgressBar1.Step = 1
	...
	else
	{
		...
		$Label8.Location = New-Object System.Drawing.Size(20,20)
		$Label8.Size = New-Object System.Drawing.Size(220,20)
		$Label8.Text = "パスワード生成中・・・"
	...
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('ComboBox1',$ComboBox1,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('NumberBox1',$NumberBox1,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('NumberBox2',$NumberBox2,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('Checkbox5',$Checkbox5,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('RandomCount',$RandomCount,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('strChars',$strChars,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('transcommand',$transcommand,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('GetRandom',$GetRandom,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('CheckBoxesCount',$CheckBoxesCount,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('GenerateRandomString',$GenerateRandomString,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('GeneratePassword',$GeneratePassword,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('eachpartchars',$eachchars,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('GetRead',$GetRead,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('Form4',$Form4,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('RadioButton1',$RadioButton1,$null)))
	$SessionState.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry('ProgressBar1',$ProgressBar1,$null)))
	...
	$Job.AddScript({
		$table = New-Object object[] ($NumberBox2.Text)
		foreach ($i in (0..($NumberBox2.Text -1)))
		{
			$password = & $GeneratePassword
			$table[$i] = New-Object PSObject -Property @{
				"No." = $i + 1
				"パスワード" = $password
				"読み方" = & $GetRead($password)
			}
			if ($RadioButton1.Checked)
			{
				$Form4.Text = ("パスワード生成中・・・(" + ($i + 1) + "/" + $NumberBox2.Text + ")完了")
	...
	for ($i = 0;$i -lt $NumberBox2.Text;$i ++)
	{
...
$Label6.Location = New-Object System.Drawing.Size(80,10)
$Label6.Size = New-Object System.Drawing.Size(70,20)
$Label6.Text = "生成数："
$Label6.TextAlign = "MiddleLeft"
$Form3.Controls.Add($Label6)

$NumberBox2 = New-Object System.Windows.Forms.NumericUpDown
$NumberBox2.Location = New-Object Drawing.Size(150,10)
$NumberBox2.Size = New-Object Drawing.Size(70,20)
$NumberBox2.Minimum = 1
$NumberBox2.Maximum = 9999
$NumberBox2.Text = 100
$Form3.Controls.Add($NumberBox2)

$Label7 = New-Object System.Windows.Forms.Label
$Label7.Location = New-Object System.Drawing.Size(260,10)
$Label7.Size = New-Object System.Drawing.Size(120,20)
$Label7.Text = "プログレスバーの表示："
$Label7.TextAlign = "MiddleLeft"
$Label7.Anchor = ([System.Windows.Forms.AnchorStyles]([System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Right))
...
"各要素を最低一文字以上含める","スペース以外の各要素を均等に使用","完全ランダム" | % {[Void]$ComboBox1.Items.Add($_)}
...
$TextBox2.Size = New-Object System.Drawing.Size(500,110)
...
$GeneratePassword = {
	if ($ComboBox1.SelectedIndex.Equals(0))
	{
		if ($NumberBox1.Text -eq (checkboxescount))
		{
			$Generate_Password = Get-Random -input ([String](& $eachchars)).ToCharArray() -count $NumberBox1.Text
		}
		else
		{
			$Generate_Password = Get-Random -input ([String](& $eachchars) + [String](& $GenerateRandomString)).ToCharArray() -count $NumberBox1.Text
		}
	}
	elseif ($ComboBox1.SelectedIndex.Equals(1))
	{
	...
	{
		$Generate_Password = Get-Random -input ([String](& $GenerateRandomString)).ToCharArray() -count $NumberBox1.Text
	}
...
function transcommandgenerate()
{
	$targetcount = 0
	$commandarray = New-Object System.Collections.Generic.List[System.String]
	switch -Regex ($strChars) {"a" {$commandarray.Add(' -replace "a","えー"')}}
	switch -Regex ($strChars) {"b" {$commandarray.Add(' -replace "b","びー"')}}
	switch -Regex ($strChars) {"c" {$commandarray.Add(' -replace "c","しー"')}}
	switch -Regex ($strChars) {"d" {$commandarray.Add(' -replace "d","でぃー"')}}
	switch -Regex ($strChars) {"e" {$commandarray.Add(' -replace "e","いー"')}}
	switch -Regex ($strChars) {"f" {$commandarray.Add(' -replace "f","えふ"')}}
	switch -Regex ($strChars) {"g" {$commandarray.Add(' -replace "g","じー"')}}
	switch -Regex ($strChars) {"h" {$commandarray.Add(' -replace "h","えいち"')}}
	switch -Regex ($strChars) {"i" {$commandarray.Add(' -replace "i","あい"')}}
	switch -Regex ($strChars) {"j" {$commandarray.Add(' -replace "j","じぇい"')}}
	switch -Regex ($strChars) {"k" {$commandarray.Add(' -replace "k","けー"')}}
	switch -Regex ($strChars) {"l" {$commandarray.Add(' -replace "l","える"')}}
	switch -Regex ($strChars) {"m" {$commandarray.Add(' -replace "m","えむ"')}}
	switch -Regex ($strChars) {"n" {$commandarray.Add(' -replace "n","えぬ"')}}
	switch -Regex ($strChars) {"o" {$commandarray.Add(' -replace "o","おー"')}}
	switch -Regex ($strChars) {"p" {$commandarray.Add(' -replace "p","ぴー"')}}
	switch -Regex ($strChars) {"q" {$commandarray.Add(' -replace "q","きゅー"')}}
	switch -Regex ($strChars) {"r" {$commandarray.Add(' -replace "r","あーる"')}}
	switch -Regex ($strChars) {"s" {$commandarray.Add(' -replace "s","えす"')}}
	switch -Regex ($strChars) {"t" {$commandarray.Add(' -replace "t","てぃー"')}}
	switch -Regex ($strChars) {"u" {$commandarray.Add(' -replace "u","ゆー"')}}
	switch -Regex ($strChars) {"v" {$commandarray.Add(' -replace "v","ぶい"')}}
	switch -Regex ($strChars) {"w" {$commandarray.Add(' -replace "w","だぶりゅー"')}}
	switch -Regex ($strChars) {"x" {$commandarray.Add(' -replace "x","えっくす"')}}
	switch -Regex ($strChars) {"y" {$commandarray.Add(' -replace "y","わい"')}}
	switch -Regex ($strChars) {"z" {$commandarray.Add(' -replace "z","ぜっと"')}}
	switch -Regex ($strChars) {"0" {$commandarray.Add('.ToString().Replace("0","ぜろ")')}}
	switch -Regex ($strChars) {"1" {$commandarray.Add('.ToString().Replace("1","いち")')}}
	switch -Regex ($strChars) {"2" {$commandarray.Add('.ToString().Replace("2","にー")')}}
	switch -Regex ($strChars) {"3" {$commandarray.Add('.ToString().Replace("3","さん")')}}
	switch -Regex ($strChars) {"4" {$commandarray.Add('.ToString().Replace("4","よん")')}}
	switch -Regex ($strChars) {"5" {$commandarray.Add('.ToString().Replace("5","ご")')}}
	switch -Regex ($strChars) {"6" {$commandarray.Add('.ToString().Replace("6","ろく")')}}
	switch -Regex ($strChars) {"7" {$commandarray.Add('.ToString().Replace("7","なな")')}}
	switch -Regex ($strChars) {"8" {$commandarray.Add('.ToString().Replace("8","はち")')}}
	switch -Regex ($strChars) {"9" {$commandarray.Add('.ToString().Replace("9","きゅう")')}}
	switch -Regex ($strChars) {"'" {$commandarray.Add('.ToString().Replace("''","シングルクォート")')}}
	switch -Regex ($strChars) {"-" {$commandarray.Add('.ToString().Replace("-","ハイフン")')}}
	switch -Regex ($strChars) {"!" {$commandarray.Add('.ToString().Replace("!","エクスクラメーション")')}}
	switch -Regex ($strChars) {"""" {$commandarray.Add('.ToString().Replace("""","ダブルクォート")')}}
	switch -Regex ($strChars) {"#" {$commandarray.Add('.ToString().Replace("#","番号記号")')}}
	switch -Regex ($strChars) {"\$" {$commandarray.Add('.ToString().Replace("$","ドル記号")')}}
	switch -Regex ($strChars) {"%" {$commandarray.Add('.ToString().Replace("%","パーセント")')}}
	switch -Regex ($strChars) {"&" {$commandarray.Add('.ToString().Replace("&","アンパサンド")')}}
	switch -Regex ($strChars) {"\(" {$commandarray.Add('.ToString().Replace("(","左カッコ")')}}
	switch -Regex ($strChars) {"\)" {$commandarray.Add('.ToString().Replace(")","右カッコ")')}}
	switch -Regex ($strChars) {"\*" {$commandarray.Add('.ToString().Replace("*","アスタリスク")')}}
	switch -Regex ($strChars) {"," {$commandarray.Add('.ToString().Replace(",","カンマ")')}}
	switch -Regex ($strChars) {"\." {$commandarray.Add('.ToString().Replace(".","ピリオド")')}}
	switch -Regex ($strChars) {"/" {$commandarray.Add('.ToString().Replace("/","スラッシュ")')}}
	switch -Regex ($strChars) {":" {$commandarray.Add('.ToString().Replace(":","コロン")')}}
	switch -Regex ($strChars) {";" {$commandarray.Add('.ToString().Replace(";","セミコロン")')}}
	switch -Regex ($strChars) {"\?" {$commandarray.Add('.ToString().Replace("?","クエスチョン")')}}
	switch -Regex ($strChars) {"@" {$commandarray.Add('.ToString().Replace("@","アットマーク")')}}
	switch -Regex ($strChars) {"\[" {$commandarray.Add('.ToString().Replace("[","左角カッコ")')}}
	switch -Regex ($strChars) {"\]" {$commandarray.Add('.ToString().Replace("]","右角カッコ")')}}
	switch -Regex ($strChars) {"\^" {$commandarray.Add('.ToString().Replace("^","キャレット")')}}
	switch -Regex ($strChars) {"_" {$commandarray.Add('.ToString().Replace("_","アンダースコア")')}}
	switch -Regex ($strChars) {"``" {$commandarray.Add('.ToString().Replace("``","バッククオート")')}}
	switch -Regex ($strChars) {"{" {$commandarray.Add('.ToString().Replace("{","左中カッコ")')}}
	switch -Regex ($strChars) {"\|" {$commandarray.Add('.ToString().Replace("|","パイプライン")')}}
	switch -Regex ($strChars) {"}" {$commandarray.Add('.ToString().Replace("}","右中カッコ")')}}
	switch -Regex ($strChars) {"~" {$commandarray.Add('.ToString().Replace("~","チルダ")')}}
	switch -Regex ($strChars) {"\\" {$commandarray.Add('.ToString().Replace("\","円マーク")')}}
	switch -Regex ($strChars) {"\+" {$commandarray.Add('.ToString().Replace("+","プラス")')}}
	switch -Regex ($strChars) {"<" {$commandarray.Add('.ToString().Replace("<","左カギカッコ")')}}
	switch -Regex ($strChars) {"=" {$commandarray.Add('.ToString().Replace("=","イコール")')}}
	switch -Regex ($strChars) {">" {$commandarray.Add('.ToString().Replace(">","右カギカッコ")')}}
	switch -Regex ($strChars) {"\s" {$commandarray.Add('.ToString().Replace([String][Char]32,"スペース")')}}
	return ('return (' + ("(" * ($commandarray.count -1)) + '[String]::Join([char]0,[char[]]$str)' + [String]::Join(")",$commandarray) + ")")
}
...
function appconfig()
{
	...
	If ([string]::IsNullOrEmpty($cfg_strlength)) {$Config.AppSettings.Settings.Add("strlength", $NumberBox1.Text)} else {$cfg_strlength.Value = $NumberBox1.Text}
	...
	If ([string]::IsNullOrEmpty($cfg_quantity)) {$Config.AppSettings.Settings.Add("quantity", $NumberBox2.Text)} else {$cfg_quantity.Value = $NumberBox2.Text}
...
$GetRead = {
	param($str)
	$str = $str
	return ([string]$transcommand.Invoke()).Replace([char]0,",")
}
...
$appconfiginitial = {
	...
	$cfg_strlength = $Config.AppSettings.Settings["strlength"]
...
if ((Test-Path (Join-Path $path "PasswordGenerator.config")))
{
	...
	$NumberBox1.Text = $cfg_strlength.Value
	$ComboBox1.SelectedIndex = $cfg_useallchar.Value
	$NumberBox2.Text = $cfg_quantity.Value
